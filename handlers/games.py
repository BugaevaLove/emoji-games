...
from keyboards import main_menu, game_mode_keyboard, bet_keyboard, play_again_keyboard
...
async def game_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    game_map = {
        "🎲 Кубик": GameType.DICE, "🎯 Дартс": GameType.DARTS,
        "🎳 Боулинг": GameType.BOWLING, "⚽ Футбол": GameType.FOOTBALL,
        "🏀 Баскетбол": GameType.BASKETBALL, "🎰 Слоты": GameType.SLOTS
    }
...
async def mode_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.update({"game_type": game_type, "mode": mode})
    user = await get_user(update)
    await query.edit_message_text(
        f"{GAME_EMOJIS[game_type]} {mode.value.capitalize()}\n"
...
async def bet_selected(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Списание ставки
    await db.update_user(user_id, balance=user["balance"] - bet)
    await db.add_transaction(user_id, "bet", -bet, f"Ставка {game_type.value} {mode.value}")

    # Бросок
    msg = await query.message.chat.send_dice(emoji=GAME_EMOJIS[game_type])
    ...
    if new_level > old_level:
        ...
        await db.update_user(user_id, balance=new_balance + vip_reward)
        new_balance += vip_reward
        await db.add_transaction(user_id, "vip_reward", vip_reward, f"VIP {new_level}")

    # Ответ
    text = f"{GAME_EMOJIS[game_type]} Выпало: {value}\n"
    text += f"🎉 Выигрыш: {win_amount:.2f} USDT (x{mult:.2f})\n" if win_amount > 0 else "😢 Проигрыш\n"
    text += f"💰 Баланс: {new_balance:.2f} USDT"
    if vip_reward:
        text += f"\n\n🏆 VIP {new_level}! +{vip_reward:.2f} USDT"
...
games_conv = ConversationHandler(
    entry_points=[
        MessageHandler(filters.Regex(r"^(🎲|🎯|🎳|⚽|🏀|🎰)"), game_selected),
        CallbackQueryHandler(play_again, pattern=r"^again_")
    ],
    states={
        SELECT_MODE: [CallbackQueryHandler(mode_selected, pattern=r"^game_")],
        SELECT_BET: [CallbackQueryHandler(bet_selected, pattern=r"^bet_")]
    },
